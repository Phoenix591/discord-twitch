...
# VENV Paths
VENV_PYTHON="$INSTALL_DIR/bin/python3"
...
[ -f "$INSTALL_DIR/requirements.txt" ] && cp "$INSTALL_DIR/requirements.txt" "$BACKUP_DIR/"
...
if [ -f "$NEW_BOT_SRC" ]; then
    ...
    echo "❌ Error: Could not find bot.py in new source structure"
    exit 1
fi

# 8. INSTALL DEPENDENCIES
DEPS_CHANGED=0
NEW_REQ="$SOURCE_DIR/requirements.txt"
INSTALLED_REQ="$INSTALL_DIR/requirements.txt"

if [ -f "$NEW_REQ" ]; then
    # Check if we need to update:
    # 1. If requirements.txt is different from installed
    # 2. OR if the version tag changed (implies a general update)
    if [ ! -f "$INSTALLED_REQ" ] || ! cmp -s "$NEW_REQ" "$INSTALLED_REQ" || [ "$LATEST_TAG" != "$CURRENT_VERSION" ]; then
        echo "   📦 Updating Dependencies (pip install --upgrade)..."
        
        if [ -x "$VENV_PIP" ]; then
            $VENV_PIP install --upgrade -r "$NEW_REQ"
            install -o root -g root -m 644 "$NEW_REQ" "$INSTALLED_REQ"
            DEPS_CHANGED=1
        else
            echo "   ⚠️  Warning: Virtual environment pip not found at $VENV_PIP. Skipping deps."
        fi
    fi
fi
...
# 11. CONDITIONAL RESTART
if systemctl is-active --quiet discord-twitch; then
    # Restart if Service, Configs, Version, or Dependencies changed
    if [ $SERVICE_CHANGED -eq 1 ] || [ $S3_CFG_CHANGED -eq 1 ] || [ $S3_SECRET_CHANGED -eq 1 ] || [ $DEPS_CHANGED -eq 1 ] || [ "$LATEST_TAG" != "$CURRENT_VERSION" ]; then
